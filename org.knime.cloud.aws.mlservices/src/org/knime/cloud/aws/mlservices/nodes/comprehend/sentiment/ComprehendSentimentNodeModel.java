 ...
 */
final class ComprehendSentimentNodeModel extends BaseComprehendLangNodeModel {
    ...
    protected ComprehendOperation getOperationInstance(final CloudConnectionInformation cxnInfo,
        ...
        return new SentimentOperation(cxnInfo, textColumn, sourceLanguage, outputSpec);
    }

    @Override
    protected DataTableSpec generateOutputTableSpec(final DataTableSpec inputTableSpec) {
        final DataColumnSpec[] allColSpecs = new DataColumnSpec[5];
        allColSpecs[0] = new DataColumnSpecCreator("Sentiment", StringCell.TYPE).createSpec();
        allColSpecs[1] = new DataColumnSpecCreator("Score (mixed)", DoubleCell.TYPE).createSpec();
        allColSpecs[2] = new DataColumnSpecCreator("Score (positive)", DoubleCell.TYPE).createSpec();
        allColSpecs[3] = new DataColumnSpecCreator("Score (neutral)", DoubleCell.TYPE).createSpec();
        allColSpecs[4] = new DataColumnSpecCreator("Score (negative)", DoubleCell.TYPE).createSpec();
