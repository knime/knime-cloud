...
import org.knime.cloud.aws.comprehend.AmazonSyntaxTag;
...
/**
 * A tagger that uses the Amazon Comprehend service to detect the syntax of each sentence
 * of a <code>Document</code>. For each sentence a call is made to the Comprehend service
 * and the returned syntax elements are wrapped with their entity type as a tag.
 *
 * @author KNIME AG
 */
...
final class SyntaxTagger extends AbstractDocumentTagger {
    ...
    protected List<Tag> getTags(final String tag) {
        return Arrays.asList(AmazonSyntaxTag.stringToTag(tag));
    }
    ...
    protected List<TaggedEntity> tagEntities(final Sentence sentence) {
        ...
        DetectSyntaxResult result = m_client.detectSyntax(request);
