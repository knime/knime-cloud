...
import org.knime.bigdata.fileformats.orc.OrcFileFormatWriter;
...
/**
 * {@link DBMultiFileWriter} implementation that writes out a Parquet file.
 *
 * @author Tobias Koetter, KNIME GmbH, Konstanz, Germany
 */
public class RedshiftORCWriter extends DBMultiFileWriter<RedshiftLoaderNodeSettings, RedshiftLoaderSettings> {

    private static final Map<SQLType, TypeDescription> REDSHIFT_TO_ORC_TYPE_MAPPING;

    static {
        final Map<SQLType, TypeDescription> orcMap = new HashMap<>();
        orcMap.put(JDBCType.BIT, TypeDescription.createBoolean());
        orcMap.put(JDBCType.DOUBLE, TypeDescription.createDouble());
        orcMap.put(JDBCType.INTEGER, TypeDescription.createInt());
        orcMap.put(JDBCType.BIGINT, TypeDescription.createLong());
        orcMap.put(JDBCType.VARCHAR, TypeDescription.createString());
        orcMap.put(JDBCType.DATE, TypeDescription.createDate());
        orcMap.put(JDBCType.TIME, TypeDescription.createString());
        orcMap.put(JDBCType.TIMESTAMP, TypeDescription.createTimestamp());
        REDSHIFT_TO_ORC_TYPE_MAPPING = Collections.unmodifiableMap(orcMap);
    }

    private DataTypeMappingConfiguration<TypeDescription> m_typeMappingConfiguration;

    private DataTableSpec m_spec;

    private CompressionKind m_compression;

    private int m_chunkSize;

    private long m_fileSize;

    private static DataTypeMappingConfiguration<TypeDescription> createORCTypeMappingConfiguration(
        ...
        final ORCTypeMappingService typeMappingService = ORCTypeMappingService.getInstance();
        final DataTypeMappingConfiguration<TypeDescription> result =
            typeMappingService.createMappingConfiguration(DataTypeMappingDirection.KNIME_TO_EXTERNAL);
        int columnIndex = 0;
        for (final DBColumn column : targetTableSpec) {
            final SQLType sqlType = column.getColumnType();
            final TypeDescription orcType = REDSHIFT_TO_ORC_TYPE_MAPPING.get(sqlType);
            if (orcType == null) {
                if (JDBCType.NUMERIC.equals(sqlType)) {
                    throw new SQLException("ORC not supported as exchange format for Redshift tables with "
                        + "numeric/decimal columns. Please use CSV instead.");
                }
                throw new InvalidSettingsException("ORC type could not be found for the SQL type: " + sqlType);
            }
            ...
            result.addRule(inputColumnType,
                typeMappingService.getConsumptionPathsFor(inputColumnType).stream()
                    .filter(path -> path.getConsumerFactory().getDestinationType().equals(orcType)).findFirst()
                    .orElseThrow(() -> new SQLException("Consumption path could not be found from " + inputColumnType
                        + " through " + orcType + " to " + sqlType + '.')));
        }
        return result;
    }
    ...
    protected String getFilenameSuffix() {
        ...
        switch (m_compression) {
            case SNAPPY:
                compressionSuffix = ".snappy";
                break;
            case NONE:
                compressionSuffix = "";
                break;
            case LZO:
                compressionSuffix = ".lzo";
                break;
            case LZ4:
                compressionSuffix = ".lz4";
                break;
            case ZLIB:
                compressionSuffix = ".zlib";
                break;
            default:
                compressionSuffix = "";
                break;
        }
        return ".orc" + compressionSuffix;
    }
    ...
    protected void setup(final ExecutionMonitor exec, final ExecutionParameters<RedshiftLoaderNodeSettings> parameters)
        ...
        if (RedshiftLoaderFileFormat.NONE_COMPRESSION.equals(compressionName)) {
            m_compression = CompressionKind.NONE;
        } else {
            m_compression = CompressionKind.valueOf(compressionName);
        }
        m_chunkSize = cs.getChunkSizeModel().getIntValue();
        m_fileSize = cs.getFileSizeModel().getLongValue();
        m_typeMappingConfiguration = createORCTypeMappingConfiguration(m_spec, table, session, exec);
    }

    @SuppressWarnings("resource")
    @Override
    protected DBRowWriter getRowWriter(final ExecutionMonitor exec, final FSPath tempFolder) throws Exception {
        final OrcFileFormatWriter writer = new OrcFileFormatWriter(tempFolder, m_spec, m_fileSize, m_chunkSize,
            m_compression, m_typeMappingConfiguration);
